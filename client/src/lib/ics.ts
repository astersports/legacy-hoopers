...
export function downloadSchedule(events: ScheduleEvent[], filename = "aster-aau-schedule.ics") {
  const lines = ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Aster Sports//Schedule//EN", "CALSCALE:GREGORIAN"];
...
export async function shareSchedule() {
  ...
  if (navigator.share) {
    try { await navigator.share({ title: "Aster Sports Schedule", url }); return; } catch { /* cancelled */ }
  }
